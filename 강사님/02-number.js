...
console.log(parseInt("011"));           // "011" -> int형 변환
console.log(parseInt("011", 2));        // 2진수 011 -> int형 변환
console.log(parseInt("123.456"));       // 정수부분만 변환
console.log(parseFloat("123.456"));     // 실수 전체 변환

// 수치 형태가 아닌 문자열의 변환 -> NaN(Not a Number)
console.log("a123 -> int:", parseInt("a123"));      // NaN 출력

let v = parseInt("a123");
console.log(v, " is ", typeof v);   // NaN도 number형이긴 하다

// 수치 변수가 NaN인지 확인 -> isNaN 함수
console.log("v is NaN? ", isNaN(v));

// NaN가 포함된 산술 계산식은 NaN
console.log("NaN 포함 산술 계산: ", v + 10);    // -> NaN으로 통합됨
